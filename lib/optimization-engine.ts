...
// --- FINANCIAL ASSUMPTIONS & BENCHMARKS ---

/**
 * Assumes 2 hours of engineering onboarding/context switching at an average rate of $75/hr.
 * This introduces friction that must be recouped via savings within 3 months to justify the change.
 */
const SWITCHING_COST_COPILOT_TO_CURSOR_USD = 150;

/**
 * Industry benchmark estimate: 25% of developer API spend is highly repetitive 
 * (e.g. running the same test suite or recompiling the same components).
 */
const PROMPT_CACHING_REPEAT_PORTION = 0.25;

/**
 * Caching repetitive prompts generally yields an 85% discount relative to full inference costs.
 */
const PROMPT_CACHING_SAVINGS_RATE = 0.85;

/**
 * Estimated fixed cost of implementing prompt caching architecture, including engineering time.
 */
const PROMPT_CACHING_IMPLEMENTATION_COST_USD = 1200;

/**
 * Waste Score Weights
 */
const WASTE_WEIGHT_OVERSPEND = 0.3;
const WASTE_WEIGHT_REDUNDANCY = 0.4;
const WASTE_WEIGHT_UNUSED = 0.3;

// ------------------------------------------

const TOOL_PLAN_PRICING: Record<string, Record<string, PricingTier>> = pricingData;
...
function assessCursorCopilotOverlap(
  ...
  const switchingCost = teamSize * SWITCHING_COST_COPILOT_TO_CURSOR_USD;
...
function checkTokenOptimization(config: ToolConfig): Recommendation | null {
  ...
  if (isAPITool && config.monthlySpend > 200) {
    const repeatablePortion = PROMPT_CACHING_REPEAT_PORTION;
    const cacheEligibleSpend = config.monthlySpend * repeatablePortion;
    const realisticSavings = Math.round(cacheEligibleSpend * PROMPT_CACHING_SAVINGS_RATE); // 85% on repeated prompts
    const implementationCost = PROMPT_CACHING_IMPLEMENTATION_COST_USD;
...
function calculateWasteScore(tools: ToolConfig[]): { score: number; category: string; breakdown: string } {
  ...
  const score = Math.min(1, overspendPct * WASTE_WEIGHT_OVERSPEND + redundancyRatio * WASTE_WEIGHT_REDUNDANCY + unusedPct * WASTE_WEIGHT_UNUSED);
  const category = score > 0.6 ? 'High Waste' : score >= 0.3 ? 'Moderate Waste' : 'Optimized';
  const breakdown = `waste score = (${(overspendPct * 100).toFixed(0)}% overspend * ${WASTE_WEIGHT_OVERSPEND}) + (${(redundancyRatio * 100).toFixed(0)}% redundancy * ${WASTE_WEIGHT_REDUNDANCY}) + (${(unusedPct * 100).toFixed(0)}% unused * ${WASTE_WEIGHT_UNUSED}) = ${(score * 100).toFixed(0)}%`;
